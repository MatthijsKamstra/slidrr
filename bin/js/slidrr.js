...
Main.prototype = {
	...
	,move: function(dir) {
		...
		if(this.shouldUpdateNotes && this.notesPopup != null) this.notesPopup.postMessage("slide:" + this._currentId,"*");
	...
	,toggleHelp: function() {
		haxe_Log.trace("toggleHelp",{ fileName : "Main.hx", lineNumber : 376, className : "Main", methodName : "toggleHelp"});
	...
	,showSpeakerNotes: function() {
		haxe_Log.trace("showSpeakerNotes",{ fileName : "Main.hx", lineNumber : 417, className : "Main", methodName : "showSpeakerNotes"});
		this.shouldUpdateNotes = true;
		...
		this.notesPopup = this._win.open("","Notes::","width=1100,height=700");
		this.notesPopup.document.write(html);
		this._win.addEventListener("message",function(event) {
			haxe_Log.trace("received response: ",{ fileName : "Main.hx", lineNumber : 487, className : "Main", methodName : "showSpeakerNotes", customParams : [event.data]});
		},false);
	}
	...
	,onClickHandler: function(e) {
		...
		if(temp.className.indexOf("left") != -1) {
			haxe_Log.trace("left",{ fileName : "Main.hx", lineNumber : 523, className : "Main", methodName : "onClickHandler"});
			this.move(-1);
		} else {
			haxe_Log.trace("right",{ fileName : "Main.hx", lineNumber : 526, className : "Main", methodName : "onClickHandler"});
			this.move(1);
		}
	}
...
var view_SlidrrView = function(md,el,slideId) {
	...
	if(vo.url != "") {
		...
		if(StringTools.endsWith(vo.url,"?")) div.className += " slidrr-fullscreen slidrr-fullscreen-contain glow";
...
var view_SpeakrrNotesView = function(md) {
	...
	this._win.onmessage = function(event) {
		var curId = Std.parseInt(event.data.split(":")[1]);
		haxe_Log.trace("should change slide to " + curId,{ fileName : "SpeakrrNotesView.hx", lineNumber : 43, className : "view.SpeakrrNotesView", methodName : "new"});
		var _u = _g._doc.URL.split("#");
		_g._doc.location.href = _u[0] + "#/" + curId;
		_g.buildNotes(md);
	};
};
...
view_SpeakrrNotesView.prototype = {
	clearNodes: function() {
		if(this.slideCurrentContainer == null || this.slideNextContainer == null) return;
		this.slideCurrent.removeChild(this.slideCurrentContainer);
		this.slideNext.removeChild(this.slideNextContainer);
	}
	,buildNotes: function(md) {
		var _g = this;
		this.clearNodes();
		...
		var curId = this.getSlideId();
		var nextId = curId + 1;
		if(curId == this.totalSlides) nextId = 0;
		var currentSlide = new view_SlidrrView(md,this.slideCurrentContainer,curId);
		var nextSlide = new view_SlidrrView(md,this.slideNextContainer,nextId);
